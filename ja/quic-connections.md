...
コネクション ID を利用することにより、TCP が絶対にできない方法で、IP アドレスとネットワークインターフェースの間でコネクションを移行することができます。 ユーザーがデバイスの場所をWi-Fiが提供される場所に移動するとき、例えばマイグレーションのおかげで、進行中のダウンロードはモバイルネットワークからより速いWi-Fiに変更できます。同様に、もしWi-Fiが利用不可能になったならば、ダウンロードはモバイルネットワークを経由して再開できます。
