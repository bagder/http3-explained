...
接続 ID を利用することにより、TCP が絶対にできない方法で、IP アドレスとネットワークインターフェースの間で接続を移行することができます。 ユーザーがデバイスの場所をWi-Fiが提供される場所に移動するとき、例えばマイグレーションのおかげで、進行中のダウンロードはセルラーネットワーク接続からより速いWi-Fiに変更できます。同様に、もしWi-Fiが利用不可能になったならば、ダウンロードはセルラー接続を経由して再開できます。
